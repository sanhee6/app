      ...
      <el-col :span="12">
        ...
        <div v-else>
          <el-form-item class="input" label="课程名称" prop="kechengmingcheng">
              <el-input v-model="ruleForm.kechengmingcheng" 
                id="kejian-kechengmingcheng-info"
...
export default {
  ...
  methods: {
    ...
    // 初始化
    init(id,type) {
      console.log('初始化课件信息表单，ID:', id, '类型:', type);
      if (id) {
        this.id = id;
        this.type = type;
      }
      ...
      setTimeout(() => {
        ...
        if (!editorElement && this.type != 'info') {
          console.warn('编辑器可能未正常加载，尝试重新初始化');
    ...
    forceReloadEditor() {
      // 保存当前内容
      const currentContent = this.ruleForm.kejianneirong;
      
      // 临时清空内容以触发编辑器重新渲染
      this.ruleForm.kejianneirong = '';
      
      // 稍后恢复内容
      setTimeout(() => {
        this.ruleForm.kejianneirong = currentContent;
        ...
        setTimeout(() => {
          const editorElement = document.querySelector('.editor .ql-editor');
          if (!editorElement && this.type != 'info') {
            console.error('编辑器仍未正常加载，可能需要刷新页面');
            this.$message.error('编辑器加载异常，请尝试刷新页面');
          }
        }, 500);
      }, 200);
    },
